# Integrating FlowClassifier

## From Solidity

```solidity
import {FlowClassifierHook} from "flow-classifier/src/hooks/FlowClassifierHook.sol";
...
const hook = getHook("flow-classifier");
const address = hookAddress("flow-classifier", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "FlowClassifier"
...
More at https://flow-classifier.pages.dev.
