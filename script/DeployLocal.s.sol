...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(FlowClassifierHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new FlowClassifierHook{salt: salt}(IPoolManager(address(manager)));
