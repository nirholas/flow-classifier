...
/**
 * @title DeployFlowClassifier
 * @notice Deploys FlowClassifier deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployFlowClassifier is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(FlowClassifierHook).creationCode;
        ...
        console2.log("FlowClassifier ", predicted);
        ...
        vm.startBroadcast();
        FlowClassifierHook hook = new FlowClassifierHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("FlowClassifier")),
